"""
Dutch document language-rewriter pipeline for nested HTML fragments.

This version combines the nested-safe extraction strategy with optional spaCy
checks after the Stage 1 plain-text rewrite:
  ...
  HTML fragment
    ...
    -> optional spaCy check of Stage 1 plain text
    -> optional Stage 1 repair of blocks flagged by spaCy
...
# spaCy is used only for optional Stage 1 quality checks. It is NOT used for
# HTML parsing/tokenization. Keep sentence-length checks disabled as requested;
# you can add them later in check_stage1_rules_spacy() if you want.
ENABLE_SPACY_STAGE1_CHECKS = True
SPACY_MODEL = "nl_core_news_lg"
STAGE1_RULE_REPAIR_PASSES = 1

# Conservative checks. Sentence-length checking is intentionally omitted.
CHECK_PASSIVE_VOICE = True
CHECK_COMPLEX_SUBCLAUSES = False
...
STAGE1_RULE_REPAIR_SYSTEM_PROMPT = f"""Je bent een gespecialiseerde tekstredacteur voor interne bedrijfsdocumenten in het Nederlands.

{GENERATIVE_RULES}

Je krijgt tekstblokken waarvan de Stage-1 tekst door een automatische spaCy-controle
is gemarkeerd. Verbeter alleen de tekst in "current_rewrite" en los de genoemde
issues op.
...
- Behoud betekenis, cijfers, namen, data en technische termen volledig.
- Verwijder geen inhoud.
- Repareer alleen wat nodig is om de genoemde issues op te lossen.
...
class Block:
    ...
    stage1_rule_issues: list[str] = field(default_factory=list)
...
# ---------------------------------------------------------------------------
# Step 2b: Optional spaCy Stage 1 checks and repairs
# ---------------------------------------------------------------------------

def load_spacy_model(model_name: str = SPACY_MODEL):
    """
    Load spaCy lazily so importing this module does not require spaCy.
    ...
    try:
        ...
        raise RuntimeError("Install spaCy first: pip install spacy") from exc
...
def _token_has_verbform_participle(token) -> bool:
    """Return True if spaCy marks this token as a participle."""
    try:
...
def _looks_like_dutch_passive(sent) -> bool:
    """
    Conservative Dutch passive-voice heuristic.

    This is intentionally not a broad grammar checker. It looks for a form of
    'worden' or 'zijn' used near a participle and/or a 'door'-phrase.
    """
    aux_lemmas = {"worden", "zijn"}
    has_passive_aux = any(
        token.lemma_.lower() in aux_lemmas
        and (token.pos_ in {"AUX", "VERB"} or token.dep_.startswith("aux"))
        for token in sent
    )
    has_participle = any(_token_has_verbform_participle(token) for token in sent)
    has_door_phrase = any(token.text.lower() == "door" for token in sent)
    has_spacy_passive_dep = any("pass" in token.dep_.lower() for token in sent)

    return bool(has_spacy_passive_dep or (has_passive_aux and has_participle and has_door_phrase))


def _complex_subclause_count(sent) -> int:
    """Optional rough count of subordinate-clause markers. Disabled by default."""
    markers = {
        "dat", "omdat", "doordat", "zodat", "terwijl", "als", "wanneer",
        "indien", "hoewel", "voordat", "nadat", "waarbij", "waarin", "waardoor",
    }
    return sum(1 for token in sent if token.text.lower() in markers or token.dep_ == "mark")


def check_stage1_rules_spacy(text: str, nlp) -> list[str]:
    """
    Check Stage 1 plain text with spaCy.

    Sentence-length checks are intentionally not included. This function is kept
    small on purpose, so you can add stricter checks later without changing the
    rest of the pipeline.
    """
    clean = TOKEN_RE.sub("", text or "").strip()
    if not clean:
        return ["stage1_empty_text"]
    ...
    for sent in doc.sents:
        ...
        if CHECK_PASSIVE_VOICE and _looks_like_dutch_passive(sent):
            issues.append(f"possible_passive_voice: '{sentence_text}'")

        if CHECK_COMPLEX_SUBCLAUSES:
            count = _complex_subclause_count(sent)
            if count > 1:
                issues.append(f"possibly_complex_subclauses_{count}: '{sentence_text}'")

    return issues


def collect_stage1_rule_failures(blocks: list[Block], nlp) -> list[Block]:
    """Run spaCy checks on Stage 1 output and return blocks that need repair."""
    candidates: list[Block] = []

    for block in blocks:
        block.stage1_rule_issues = []
        if not _valid_stage1_text(block.stage1_text):
            continue

        issues = check_stage1_rules_spacy(block.stage1_text or "", nlp)
        if issues:
            block.stage1_rule_issues = issues
            # Keep a readable trail in the final report, but avoid duplicating
            # old rule issues across repeated passes.
            block.issues = [issue for issue in block.issues if not issue.startswith("stage1_rule:")]
            block.issues.extend(f"stage1_rule: {issue}" for issue in issues)
            candidates.append(block)

    return candidates


def _stage1_rule_repair_payload_text(block: Block) -> str:
    return "\n".join([_plain_text(block), block.stage1_text or "", "\n".join(block.stage1_rule_issues)])

def repair_stage1_rule_batch(blocks: list[Block]) -> None:
    """Ask the LLM to repair Stage 1 text using the spaCy issue list."""
    payload = [
        {
            "id": block.id,
            "original_text": _plain_text(block),
            "current_rewrite": block.stage1_text or "",
            "issues": block.stage1_rule_issues,
        }
        for block in blocks
    ]

    user_prompt = (
        "Verbeter onderstaande Stage-1 blokken. Los de genoemde issues op, "
        "maar behoud betekenis en inhoud volledig:\n\n"
        + json.dumps({"blocks": payload}, ensure_ascii=False, indent=2)
    )

    result = call_openai_json(
        system_prompt=STAGE1_RULE_REPAIR_SYSTEM_PROMPT,
        user_prompt=user_prompt,
        response_schema=BLOCKS_RESPONSE_SCHEMA,
        temperature=0.1,
    )
    ...
    for block in blocks:
        ...
        if revised and revised.strip():
            ...
            block.issues.append("stage1_rule_repair_failed")

def run_stage1_spacy_repairs(
    blocks: list[Block],
    *,
    nlp,
    max_passes: int = STAGE1_RULE_REPAIR_PASSES,
) -> None:
    """Check and repair Stage 1 text with spaCy issue detection."""
    for _ in range(max_passes):
        candidates = collect_stage1_rule_failures(blocks, nlp)
        if not candidates:
            return

        for chunk in chunk_blocks_by_chars(candidates, STAGE1_MAX_CHARS_PER_BATCH, _stage1_rule_repair_payload_text):
            repair_stage1_rule_batch(chunk)

    # Final check after the last repair pass, so the report tells the reviewer
    # what remains. We do not block Stage 2 on these issues.
    collect_stage1_rule_failures(blocks, nlp)


# ---------------------------------------------------------------------------
# Step 2c: Stage 2 - markup/image placeholder reinsertion
# ---------------------------------------------------------------------------
...
def process_document(
    ...
    output_path: str,
    *,
    enable_spacy_checks: bool = ENABLE_SPACY_STAGE1_CHECKS,
    ...
    # Basic operational repair: missing/empty Stage 1 responses.
    run_stage1_empty_repairs(blocks)

    # Optional spaCy check/repair on Stage 1 plain text.
    # This happens before Stage 2 because Stage 2 should only place markup tokens.
    if enable_spacy_checks:
        nlp = load_spacy_model(spacy_model)
        run_stage1_spacy_repairs(blocks, nlp=nlp)
    ...
    report = {
        ...
        "failed": [],
        "warnings": [],
        "spacy_checks_enabled": enable_spacy_checks,
        "spacy_model": spacy_model if enable_spacy_checks else None,
    }
            ...
            report["failed"].append(
                {
                    "id": block.id,
                    "issues": block.issues,
                    "stage1_rule_issues": block.stage1_rule_issues,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Rewrite Dutch HTML fragments while preserving nested structure.")
    parser.add_argument("input_html", help="Input HTML fragment/file")
    parser.add_argument("output_html", help="Output HTML file")
    parser.add_argument(
        "--no-spacy-checks",
        action="store_true",
        help="Disable optional spaCy Stage-1 checks/repairs.",
    )
    parser.add_argument(
        "--spacy-model",
        default=SPACY_MODEL,
        help=f"spaCy model to use when checks are enabled. Default: {SPACY_MODEL}",
    )
    ...
    result = process_document(
        args.input_html,
        args.output_html,
        enable_spacy_checks=not args.no_spacy_checks,
